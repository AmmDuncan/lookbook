...
**P-FA-03. Time is contextual on feed kind (F72): activity/notification feeds use relative time ("2 days ago") + date-group headers (Today / Yesterday / Earlier / a date) once the stream is long; audit/security logs MUST use ABSOLUTE timestamps; a public changelog shows relative AND absolute together.** Activity/notification feeds **may** use absolute when the exact instant is load-bearing (shift logs, incident post-mortems, security/account events) — else relative (live-UI calibration: 7shifts/Airbnb/Deel/Better Stack ship absolute on non-audit activity feeds).
Refines: F72 (scoped). *Why:* relative time aids scanning recent items (F72), but a compliance log needs the *exact instant* — PlanetScale/Unity/GitHub all show absolute "2025-08-14 10:44:13"; and some activity feeds reference items later or are time-of-record, where absolute is the right call. Long streams group under date headers so the eye finds a boundary (Basecamp "Today", Airbnb "19 JUN 2023", Gemini per-date blocks). Webflow's "**3 months ago — October 15, 2022 at 02:23am**" is the gold standard for a changelog: relative for scan, absolute for the record (`evidence/feed.md`). *Check:* audit-log rows show an absolute timestamp; activity rows show relative (or absolute when the instant matters); nothing shows a raw machine timestamp like `1660473853`.
...
- **Relative time *as the only timestamp* on an audit/compliance log** where the exact instant is the point (P-FA-03). (Absolute time on an activity feed is fine — even good — when the instant is load-bearing.)
