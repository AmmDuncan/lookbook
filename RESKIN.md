...
5. **Manifest every primary shell surface against a specimen — none left generic.** The composition isn't just "the main content area." Enumerate the shell surfaces — **nav rail · top/context bar · table chrome · active-nav state · empty/loading/error states** — and pull EACH from its variation library, named: rail → `Sidebar Variations.html` (8 treatments), top bar → `Header Topbar Variations.html`, whole shell → `App Shell Variations.html`, table/data → `Data Display Gallery.html`. A surface left at its bare default (a generic 240px white rail, an undifferentiated topbar) is the `AP29` tell — it reads as unfinished, *next to* the surfaces you did treat. This is the step that's silently skipped: attention goes to cards/KPIs/charts and a primary shell surface regresses to generic. Record the pulls in the *Surface deliberateness* receipt line.

Output of Phase 1: a working layout in the neutral skin, with every primary shell surface traceable to a specimen variant. Run `check-layout.mjs` against sibling compositions so it isn't a re-skin of one you already have.

> **Handing the build to a sub-agent? Name the variant for every primary surface in the spec.** Sub-agents (and weaker models) default to generic on *anything* left under-specified — **under-specified surface = generic surface.** A spec that nails the skin tokens and the card/KPI treatment but says nothing about the rail will get a bare default rail back. So the dispatch spec must carry the per-surface variant picks explicitly: `rail → Sidebar Variations · variant <n>`, `top bar → Header Topbar Variations · variant <n>`, `table → Data Display Gallery · variant <n>`, plus the two locked scale ramps (type + spacing, per SKILL.md step 4). Don't assume the sub-agent will reach into the specimen libraries on its own — it won't unless told which variant, by name.
