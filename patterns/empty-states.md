...
| **403 / permission** | They're authed but not allowed | **Respectful, not a wall** | the path to access | **request access / contact** *(social)* or **upgrade / billing** *(plan-gated)* |
...
**P-ES-01. First-run empty is warm and aspirational, sells the *first creation*, and carries exactly one primary "create" CTA — for an empty *list / section*.** A **first-app-load home** with several equal entry points may instead use a **2–4 pathway-card cluster or a getting-started checklist** (live-UI calibration: Render/Mixpanel/June/Current/Gumroad ship pathway-cards/checklists, not one button). First-run warmth is **domain-scaled**: consumer = warm; infra/billing = calm-helpful. Headline names the outcome ("Create your first assessment", "No forms yet" + "let's get started"); help is one encouraging line; a ghost "Learn more" / "How it works" is the only allowed second action *on the single-CTA form*.
Refines: F33, F73, F16, F68. *Why:* this is the new user's first real screen — it must pull them into the action(s) that make the product valuable, not present an empty grid. An empty *list* has one obvious next move (one CTA); a first-load *home* legitimately offers a few equal starting paths. Evidence: Kajabi (big "Create your first assessment" headline + illustration + `Get started` primary / `Learn more` ghost), Tally ("No forms yet" / "Roll up your sleeves and let's get started" / `Create form`), Aboard ("No onboardees" / `New onboarding`), Typeform ("Come on in, jane" / `Create typeform` — warmth can go personal) — all warm, all exactly one create CTA (`evidence/empty-states.md`).
...
**P-ES-02a. Show the query that returned nothing** when the surface searched for it — "No results for 'meal'" reads far better than a bare "No results". Echo the term (and active filters) so the user sees *what* matched nothing. A **"0 of N results"** count is a valid form of the echo (live-UI calibration: Navan/Going/OpenSea).
...
**P-ES-04. 404 orients and routes home: name that the page isn't here in plain words, then one primary "back home" (or a search box) — or a short helpful-links list (Home / Search / Help).** Light warmth/personality is allowed here (a 404 is not the user's fault and not urgent) but stays brief; a reference code ("Error code: 404") is allowed as fine print (live-UI calibration: Airbnb/Dropbox use a links-list, Airbnb shows the code as fine print).
...
**P-ES-05. 403 / permission is a doorway, not a wall: state plainly that they don't have access, name the path to *get* it, and keep a ghost escape ("back to my content") — never a bare "Access denied".** Two real recovery flavors (live-UI calibration — now observed across Notion/Whop/Framer/Whereby/Udemy/Retool/Slack): **social** (request access / contact the owner-or-inviter) when the block is a sharing boundary, and **entitlement / plan-gated** (the feature isn't on the plan → "Upgrade" / "Go to Billing") when the block is commercial. Tone respectful/calm, never playful.
New (live-UI calibration: PROMOTED from reasoned to observed). *Why:* the user is legitimate (they're signed in); stranding them with "Forbidden" wastes the relationship. The recovery is either social (point them at the human who can unblock them) or commercial (point them at the upgrade) — pick by *why* they're blocked. Mirrors P-ES-02's recovery-first stance for the permission moment.
...
**P-ES-08. Exactly one *filled* primary action (or a links-list with no filled button at all); ghost/text secondaries are few (1–2). Verb labels (F68).** "Create form" / "Clear filters" / "Try again" / "Back to home" — never "OK", "Submit", or two filled buttons side by side.
Refines: F16, F68, P-F-04. *Why:* the whole point of the state is to hand over a clear next move; two *equal filled* CTAs reintroduce the decision the empty screen exists to remove (AP18). But one filled primary + 1–2 quiet ghost secondaries is fine (Notion "Request access" + "Back to my content"; Zoom "Retry" + "Report" + "Leave"), and a pure links-list with zero filled buttons (Airbnb/Dropbox 404s) is also good — the cap is on *competing filled* buttons, not on quiet secondaries (live-UI calibration; `evidence/empty-states.md`).
...
**P-ES-10. The empty/no-results/error state renders *inside the surface it belongs to*, at the right scope — full-page for 404 / a whole-route 403 / first-app-load; region-scoped for an empty list, a failed widget, a no-results table, or a single gated *feature* (a plan-gated feature renders the 403 region-scoped inside the live shell — Retool, Slack — live-UI calibration).** A failed dashboard card retries *itself*; a 404 takes the whole viewport. Don't blow a region-empty up to a full page, and don't bury a route-level 404 inside a card.
...
## Sources
Refactoring UI (Wathan & Schoger) · NN/g (empty states, error message guidelines) · F33 (four data states), F69 (error copy = problem + fix), F70 (no filler), F73 (tone matches the moment), F36/F37/F43/F48/F49 (texture & imagery on sparse surfaces), F16/F68 (one primary, verb labels) · cross-refs P-DT-10 (per-region states), P-F-04 (primary placement), AP18/AP19 · **Reference study of real public products (Tally, Aboard, Typeform, Kajabi · OpenSea, Dovetail, Mixpanel, Midday · Unsplash, Headspace, Sana AI, Google · Quicken, Udemy, Zoom) → `evidence/empty-states.md`** (six-type split, recovery-not-dead-end, tone-flexes-by-moment, one-primary-CTA, query-echo, scope, texture-earns-its-keep).
