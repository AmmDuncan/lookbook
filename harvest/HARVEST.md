...
> **DELIVERED 2026-08-09 → `Surface Relationship Studies.html`** (all 8 relations + the header sub-axis + greyscale proof; extended the same day with three axes the original ask implied but did not name — horizontal split, scroll ownership, and rail collapse). Original ask below, kept for the reasoning.
> **Requested (Ammiel, 2026-08-09):** a **SURFACE-RELATIONSHIP drill-down study** for shells — the missing axis. Every existing shell specimen separates nav from content the same way: a 1px border, sidebar flush to the viewport edge, content beside it. That is one idea drawn N times, and it is why a set of "distinct" shells reads as the same design. The axis to vary is how the nav surface RELATES to the content surface, holding nav content and page content constant: bordered (baseline) · same-level (no border, shared ground, separation by space alone) · sunken nav + raised content panel · floating content (content as an inset card with radius + shadow, gap all round, nav flush to page ground) · inset nav (nav is itself a floating card inside page padding) · ink/dark rail + detached rounded content · full-bleed content where nav is the only chrome · two-tone (nav on tinted ground, content pure white, no border). Also vary where the header sits relative to the split — spanning both, or only over content. Same rule as recipes: these are COMPOSITION, not skin, and must hold up unskinned in greyscale.
...
| **Mobile app shell** — the mobile navigation + modality family: ONE app Home screen (DVLA self-service, held constant) dropped into phone device frames (pure-CSS bezels, notch + home-indicator), varying only the shell. Variants: (1) bottom tab bar (full-width, 5 tabs, active = filled icon + accent label, no inset card) · (2) floating-island nav (dark elevated pill above the safe area, 4 items + a prominent center primary Pay button) · (3) top segment + drawer (segmented control + hamburger left drawer over a scrim; for >5 destinations) · (4) sheet-vs-modal modality (two frames side by side: bottom sheet peeking over dimmed Home w/ grabber handle vs full-screen multi-step modal w/ Cancel/title bar + step progress; + the weight rule: sheet=light/reversible/keep-context, modal=focused/multi-step/destructive) · (5) safe-area anatomy (annotated device cross-section labeling the inset stack: status-bar/notch `env(safe-area-inset-top)` · header · scroll region · bottom nav · home-indicator `env(safe-area-inset-bottom)`). All icons + status glyphs = inline SVG (75 total, offline-safe); indigo accent sparing; status colors map (Expiring=amber, Valid=green); active nav = filled-icon/bg-tint, never side-border card. Serves wallet-whiz + harmony mobile customer flows. | `Mobile App Shell Studies.html` |
...
| **Form field layout** ⭐ — the FIELDS themselves (organisation and commit live in `Long-Form Organization Studies` / `Action & Commit Placement Studies`). ONE DVLA registration form, 16 ways, across 4 axes — and **several variants are marked ✗ Avoid on purpose**, because field layout is one of the few areas with real evidence behind it. **A** column structure (single column ✓ · two columns of unrelated fields ✗ · compound row under one label ✓ · short same-topic pair △) · **B** field width as a hint (all full-width uncapped ✗ · capped measure △ · width ∝ expected answer ✓ · narrower than the answer ✗) · **C** help and error placement (proactive hint under label ✓ · format revealed only after failure ✗ · help in a side column △ · error summary + inline ✓) · **D** one input or several (known date as 3 boxes ✓ · calendar picker for a known date ✗ · phone split into segments ✗ · one flexible input + addon ✓ — the axis people get backwards). Every verdict carries its source (Baymard · NN/g · GOV.UK · Adam Silver · web.dev). | `Form Field Layout Studies.html` |
| **Role-gated UI** ⭐ — ONE capability (`deposit.verify`), ONE user who lacks it (a branch cashier who CAN be granted it), 16 renderings, verdicts marked ✓/△/✗ with sources. **Spine:** a UI gate is VISIBILITY, never the security boundary — every frame assumes the server refuses anyway. **Deciding question:** can this user ever hold this capability? Never → hide; yes → show and route. **A** the blocked control (hidden △ · disabled-no-reason ✗ · disabled + persistent inline reason △ · enabled + soft-warn-on-use ✓ = the in-repo `RoleEditor.vue` precedent) · **B** the same capability across 4 surfaces (nav item / whole page / section inside a permitted page / row action — pick ONE policy per capability, mixing teaches nothing) · **C** whole-page denial (hard 404 △ only when existence is secret · explains+routes ✓ · renders-what-is-permitted ✓ · silent redirect ✗) · **D** how far the explanation goes (nothing ✗ · names the capability △ · names who grants it ✓ · in-product request with persisted status ✓ only when grants are routine). Serves the v2 RBAC scope/flag migration. | `Role-Gated UI Studies.html` |
...
| **Surface relationship** ⭐ — FIVE axes over one constant dataset. **A** how the NAV surface relates to the CONTENT surface (bordered / same-level / sunken-nav+raised-content / floating content / inset nav / ink rail + detached / full-bleed / two-tone) · **B** header position vs the split (spanning / over-content / none) · **C** the same relations on a HORIZONTAL split (top bar, 6 variants) · **D** which plane owns the SCROLL (whole page / nav-fixed / dual independent / sticky header inside content — live overflow, parked mid-scroll) · **E** rail COLLAPSE as a relation change (open / icon rail / hover flyout / off-canvas) · plus an A×B combination guide and a greyscale proof strip | `Surface Relationship Studies.html` |

> Shell-surface tie-in: when manifesting an app screen's shell surfaces (SKILL.md step 4), the **Dashboard Shell & Widget** study is the rail-as-layout-axis reference, **Containment & Chrome Context** the in-shell-vs-inline reference, and **Surface Relationship** the nav-plane-vs-content-plane reference (pick the relation BEFORE styling either surface) — alongside the component-level `Sidebar Variations.html` / `App Shell Variations.html`.
