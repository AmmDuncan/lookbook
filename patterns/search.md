...
New. *Why:* Jira's search dropdown shows work-item matches **plus** "Filter by project / assignee / reporter / status" *inside the dropdown* — the user refines before ever landing on a results page (`evidence/search.md`). Recent/suggested on focus-empty (Mobbin's suggested-search chips) removes the cold-start blank. The dropdown is the fastest path; the full results page is the fallback for "none of these, search everything."
...
- **First-run / empty-query.** Field focused, no query yet: show **recent searches + suggested/popular queries** (chips or a short list), not a blank void. Mobbin's "Try these popular searches" chips; Jira's "recently viewed". This is *distinct* from no-results — the user hasn't asked yet.
...
New. *Why:* a first-run prompt shown after a failed search reads as the system ignoring what the user typed; a "no results for X" shown before they've typed is nonsense. Evidence: Mobbin and Twist clearly separate the two; the strongest empties (Mobbin) put *suggested queries* in both but change the framing copy.
