...
**AP26. Over-composed molecule — a "component" that freezes a layout (the mini-organism in disguise).**
*Tell:* A reusable unit that bakes in *an arrangement*, not just a treatment — e.g. a "KPI card" that is permanently "hero number + exactly four stats in a row under a hairline." Every screen that reaches for it reproduces that one layout.
*Why it reads as AI:* It looks like a design system, but it homogenizes — the same prominent block recurs across otherwise-different screens. The taste-bearing atom (how the number looks) got fused to a composition decision (where the supporting stats sit), so composition can't vary. (See `patterns/molecules.md` P-MOL-01.)
*Instead:* Apply the granularity test — *does it dictate where things sit relative to each other?* If yes, split the treatment into an atom (`hero-figure`, `stat`) and name the arrangement as one of several recipes (`hero + stat-row`, `hero-solo`, `stat-tiles`…). The treatment recurs (coherence); the recipe varies with the brief (variety). (Caught by eye 2026-05-30: three same-brief composes produced an identical summary block until the figure molecule was re-atomized.)

**AP27. Near-duplicate recipes faking variety — boxed-vs-bare as if it were two layouts.**
*Tell:* A recipe menu padded with options that differ only by a card border or by density (compact vs roomy), presented as genuine compositional range.
*Why it reads as AI:* A builder "choosing" between them still ships the same shape — the variety is cosmetic. Real range comes from elements sitting in different *places*, not from the same shape boxed or unboxed.
*Instead:* Make boxing/density **flags** on a single recipe; reserve "recipe" for structurally distinct arrangements (stats in a row vs a rail vs as tiles vs absent). (See `patterns/molecules.md` P-MOL-02. Caught by eye 2026-05-30.)

**AP28. Decorative status-cosplay — the live-dot + uppercase tracked-mono micro-label.**
*Tell:* A small colored dot beside an ALL-CAPS, letter-spaced, monospace label, implying a live / real-time / "technical" state that isn't real — `● LIVE`, `● streaming`, a `· live` eyebrow on a static figure, `FRONTEND ENGINEER · ACCRA / REMOTE`, `● Available`, `● Building X`, role/eyebrow lines set in tracked mono caps with middot separators.
*Why it reads as AI:* It's status theatre. The dot signals a live channel that doesn't exist (a portfolio isn't "live"; a hero revenue number isn't "streaming"), and mono + tracked caps + middots are a reflex "make it look like a dev tool / make it look designed" costume carrying zero information (violates F17 accent-scarcity, F19 accent-encodes-meaning, F13 noise-tax). It's one of the most recognizable Claude/AI-generated signatures on dev portfolios, SaaS heroes, and dashboards — and it slips past a generous eye precisely because it *feels* polished.
*Instead:* A live indicator is legitimate ONLY when it flags **genuinely live, changing state** — an actual event stream, a deploy building, a real "open to work" toggle the person maintains — and then one quiet instance, not a pile. For a label that's just a role, location, or section tag, use plain treated type (a weight, a size, a tonal color, a real word) — not a dot, not tracked-mono-caps, not a middot chain. Mono is for things that are literally code/data (a snippet, a token, a metric), not for cosplaying technical-ness. (Caught by the USER's eye 2026-06-02 — the tell was UNNAMED here, shipped live in two kits (`cool-technical/atom-figure` "· live", `marketing/atom-code` "● streaming"/"● live"), AND reproduced in a fresh portfolio build, with every self-judged convergence test rating it "production-grade." The lesson: a generous self-eye passes status-cosplay; this rule exists so the brain catches it without needing a human to.)

**AP29. Generic shell chrome — a primary shell surface left at its bare default.**
*Tell:* The "obvious" surfaces are skinned deliberately (cards, KPI tiles, charts all pulled to the system) but a **primary shell surface** regressed to a generic default — a bare white `240px` nav rail, an undifferentiated topbar, a default `<table>` with no chrome treatment, an unstyled empty/active state. The deliberate surfaces and the generic one sit side by side, so the page reads as half-finished scaffold.
*Why it reads as AI:* It's the **inverse** of AP25 — not over-decoration, *under-attention*. A model (especially when under-specifying a sub-agent spec) spends its design attention on the surfaces it thinks of as "the design" and lets secondary-but-important shell surfaces fall to the most-probable default. The variation libraries that would make them deliberate (`Sidebar Variations.html`, `Header Topbar Variations.html`, `App Shell Variations.html`, `Data Display Gallery.html`) exist and are good — they just weren't reached into, because nothing forced the pull. A bare rail isn't "clean," it's *un-decided*.
*Instead:* Run the **shell-surface manifest** (SKILL.md protocol step 4): enumerate nav rail · top bar · table chrome · active-nav · empty/loading/error and pull EACH from its specimen variant, named in the *Surface deliberateness* receipt line. "Default / generic" is not an allowed answer for a primary surface; an explicit "none needed — <reason>" is. (Reported from a real build 2026-06-27: every layer pulled deliberately except the left rail, left at "240px, light surface" — a bare white sidebar that read as unfinished, while the `Sidebar Variations` library with 8 deliberate rail treatments went untouched.)
*Not this tell:* a rail/topbar/table that *was* pulled from a specimen and is deliberately minimal (a chosen slim icon-rail, a chosen borderless table) — minimal-by-decision is fine; the tell is generic-by-default. And on a genuinely chrome-free surface (a focused single-card auth screen, a full-bleed marketing page) there is no shell to manifest — N/A, stated.
