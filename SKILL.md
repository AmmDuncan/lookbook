...
- **`cookbooks/` — recipes that demonstrably cook.** Each cookbook is a surface recipe **earned by reproducing a real screen** (not self-authored, not convergence-graded). Seventeen so far — app: `dense-data-list`, `dashboard-kpi`; marketing: `marketing-hero`, `pricing-table`, `synthetic-product-imagery`; system: `settings-page`, `command-palette` (+ the reusable overlay shell), `data-table`, `details-rail`, `modal-and-repeater`, `forms` (the input furniture — control inventory × full state matrix with focus as a first-class state; the recipe that owns `atom-field`/`recipe-form` so builds inherit focus instead of shipping bare controls), `charts`, `ops-console` (dense operational surface — metrics strip + status grid + streaming logs as one console, status-as-shape at log-table scale); content: `reading-surface` (long-form article — the first non-scanning archetype) and `editorial-index` (the curated listing that leads to articles — chrome + featured lead + weight-varied ruled column grid + stacked department); cross-cutting: `state-as-shape` (the status-cosplay antidote) and **`registers`** — voice as a *dial-space* (type · serif-character · surface · temperature · density · depth · composition · accent · data/number type), with five worked voices on constant content. When the surface you're building matches a cookbook, load it — it beats the older `patterns/*.md` for that surface because it was proven against reality. Cookbooks **grow by coverage**; the old self-judged `patterns/` and `kits/` are superseded surface-by-surface as cookbooks reach them, never in a big-bang. (`kits/` is **conceptually** superseded by `cookbooks/registers.md` — the fixed-3-registers model is dead — but its *artifact layer*, the per-voice `_tokens.css` + copy-paste atom/recipe/organism specimens, has no empirical replacement yet; treat those specimens as untrusted reference — they're the layer that shipped AP28 — until re-earned through the reproduction gate.)
- **The pre-ship checklist + adversarial critic (`cookbooks/registers.md`) — mandatory before showing anyone.** The builder's own eye runs generous (proven: it rated a whole set "done" in which an independent critic then found blockers in every file — WCAG-failing text, fake variety, §11 tells). Before presenting designed output, run the registers pre-ship checklist (contrast floors incl. the faint tier · every interactive element has a visible keyboard focus indicator · numbers in a tabular companion, never the display face · no status-cosplay · composition moved, not just palette · imagery real-or-product, never fabricated illustration) and, for anything substantial, an adversarial critic pass (a fresh agent prompted to find flaws and forbidden to praise). **Three checklist lines are now deterministic gates:** `node scripts/check-contrast.mjs <file>` (WCAG-AA, incl. the faint tier — checks LIGHT and DARK independently when a dark override exists, so dark-mode-only failures don't slip through; a *warning* on faint-vs-elevated-surface is a real failure if any faint text renders there), `node scripts/check-layout.mjs <a> <b> …` (fingerprints layout geometry, flags shared skeletons = the fake-variety/re-skin tell), and `node scripts/check-focus.mjs <file>` (fails on any interactive element whose focus ring is suppressed without replacement — the bare-button bug a real build shipped). Run all three before the critic. The work is done when it survives an adversary, not when it looks good to its author.
...
- **Engagement ethics floor** (whenever the surface ships to real users — especially any signup / checkout / subscription / cancellation / notification / retention mechanic). Load `patterns/engagement.md` and confirm the floor: **no deceptive pattern** (P-EN-11 — roach motel, confirmshaming, trick questions, sneak-into-basket, forced continuity, nagging, obstruction, preselection, hidden costs, fake urgency/scarcity/social-proof), and **every engagement mechanic passes the five manipulation tests** (P-EN-10 — informed-consent · whose-interest · bias-direction · transparency · exit-path). Scope is **production**: placeholder counts/scarcity in a mockup are fine (the line is deceiving a *real user at ship time*). If the surface has no engagement/commercial mechanic, this reduces to the exit-path check and is otherwise N/A.
