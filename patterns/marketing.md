...
New (battle-test 2026-05-31 — derived from Linear / Stripe / Vercel / Mercury, against the `kits/marketing` landing organism). *Why:* an all-one-background landing whose sections are divided by hairline rules reads FLAT even when the content varies — and the hairline-between-sections is the cool-technical/app/doc tell leaking onto a marketing surface. Real premium landings give each argument its own ground: a tinted testimonial band, a white feature band, a dark CTA. The fix that proved it (a flat paper page with `border-top` separators → paper→white→tint→paper→dark bands, hairlines removed) measurably lifted the rhythm. **The band delta must be PERCEPTIBLE at a glance** — a ~5% accent wash or pure-white-on-warm-paper is too subtle, reads as "same background", and the rhythm doesn't register; reach for a clearly distinct tint (≥~6–8% / a soft cool tint against warm paper) or a real surface change. *(Calibrated 2026-05-31: the first pass used a 5% `--accent-faint` tint that was invisible in the render — bumped to a perceptible soft-indigo band.)* *Check:* count distinct section-background treatments down the page (≥2); each must be visibly distinct from its neighbour at a glance; confirm no `border-top` hairline separates two stacked full-width marketing sections.

**P-M-17. Hero and section headlines use `text-wrap: balance` so lines wrap evenly — never a lone orphan word on the last line.** A display headline that drops one word ("…actually / trusts.") onto its own line reads as unconsidered. Balance the wrap (CSS `text-wrap: balance`) on every hero/section headline; keep the `max-width` in `ch` so the balancer has room to work.
New (battle-test 2026-05-31). *Why:* premium marketing headlines are visibly balanced — an orphaned last word is a small but constant "generated" tell on the highest-prominence type. *Check:* every hero/section headline sets `text-wrap: balance`; no rendered headline ends in a single orphaned short word.

**P-M-18. The hero/feature product-visual is PRODUCT-TYPE-FIT, not always a dashboard. SaaS → an app dashboard in a browser frame; dev tool → a terminal / live-log stream / syntax-highlighted code block; consumer → a device frame; data product → a chart/viz. All share the framed-with-depth treatment (P-M-13); code/terminal syntax colors are TOKEN-DERIVED (`--code-*`), never invented per build.**
New (convergence test 2026-05-31 — a fresh dev-tool build had to INVENT a terminal hero + a code-block palette because the kit only modelled a SaaS dashboard). *Why:* the hero must SHOW the *real* product (P-M-13); for a developer tool the real product is its API / logs / code, so a dashboard mock is a lie, and a per-build syntax palette drifts off-register. The kit ships `atom-code` (terminal + code-block) for this, with `--code-*` keeping syntax on-register (keyword = indigo, ties to the accent). A quote that needs proof pairs with a metric row (the `testimonial + metrics` treatment in `atom-social-faq`), not a lone centered quote. *Check:* the hero visual matches the product's actual surface; any code is syntax-colored from `--code-*`, not ad-hoc hex.
...
- **Mobile** drops to one column always; hero text shrinks first, then deck, then visual. **Stacked CTAs stay content-width and centered — never stretched full-bleed** (a pill-radius button at full container width reads as a giant stretched lozenge; `align-items: center` on the stacked cluster, not the default `stretch`). Battle-test 2026-05-31.
