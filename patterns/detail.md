...
**P-DT-01. Identity header: avatar/icon (40–64px) + title + status pill on one line, key meta beneath or inline, breadcrumb above, primary action top-right.** Title 24–32px weight 600 (F4). The status pill sits immediately beside the title, not in a far corner.
...
**P-DT-03. Layout is contextual — this is the dynamic-generation branch. Pick by how much the record carries:**
...
- **Three-column (CRM-rich records).** Left rail = identity + core fields, center = activity/notes, right rail = related records (deals, tickets, companies). *Use when* the record is a hub with many linked entities and a heavy activity stream (HubSpot, Salesforce, Apollo).
...
**P-DT-07. History is a single vertical activity timeline: a connector line, one dot/icon per event, actor + action + relative timestamp per row, newest grouped under a date header.** Timestamps relative ("2 days ago") with absolute on hover/aside (F72). Icon encodes event type, not decoration (F44).
New. *Why:* every reference screen with history uses the same vertical timeline (HubSpot, Salesforce, Shopify, folk, Linear, Customer.io activity log). It's the canonical "what happened to this record" device; a table of events reads as raw data, a timeline reads as a story. Long timelines scroll within their region (overflow-scroll), not the page.
...
**P-DT-09. Editable fields edit in place (inline pencil / click-to-edit) — not a jump to a separate edit page — for single-field changes.** A full "Edit" mode/page is for multi-field restructuring. Inline edits validate on blur (P-F-07) and show field-level errors under the field (P-F-06).
New. *Why:* the detail page is read-mostly with *targeted* edits; HubSpot/Salesforce/folk all expose per-field inline edit (pencil affordance) so the user changes one fact without losing context. Bouncing to a form page for a one-field change is friction.
...
- **History rendered as a raw table** instead of a timeline (P-DT-07).
